...
func selfDisjointSet(data int) *Node {
	node := &Node{}
	node.data = data
	node.parent = node
	node.rank = 0
...
func disjointSetMap(n int) map[int]*Node {
...
func main() {
	fmt.Println("Implementation of Graph theory for Journey to the Moon in Hacker rank")
	var p, q int
	fmt.Scan(&p, &q)
	fmt.Println(FindAstronautsForMoonMission(p, q))
